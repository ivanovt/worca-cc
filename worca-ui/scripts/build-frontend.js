...
async function run() {
  ...
  const sharedSchemas = [
    ...
    // Reserved env-key denylist: Python (worca.utils.env) loads the same
    // file via importlib.resources — single source of truth, no drift.
    [
      path.join(
        repoRoot,
        '..',
        ...
        'worca',
        'schemas',
        'reserved_env_keys.json',
      ),
      path.join(serverSchemasDir, 'reserved-env-keys.json'),
    ],
  ];
  ...
  // Codegen: derive event-constants.js from the server renderer registry.
  // The Tier 1 set is *defined by* renderers.js (docs/events.md §Chat-rendered)
  // — the app must never hand-maintain its own copy, which silently drifted
  // (it listed pipeline.run.cancelled, which has no renderer).
  const eventConstantsOut = path.join(utilsDir, 'event-constants.js');
  try {
    const renderers = await import('../server/integrations/renderers.js');
    const tier1 = [...renderers.TIER1_EVENTS].sort();
    const optIn = Object.keys(renderers.OPT_IN_RENDERERS).sort();
    const lines = [
      '// AUTO-GENERATED by scripts/build-frontend.js — do not edit.',
      '// Source: server/integrations/renderers.js (the Tier 1 registry).',
      '',
      `export const TIER1_EVENTS = Object.freeze(${JSON.stringify(tier1, null, 2)});`,
      '',
      `export const OPT_IN_EVENTS = Object.freeze(${JSON.stringify(optIn, null, 2)});`,
    ];
    writeFileSync(eventConstantsOut, `${lines.join('\n')}\n`);
    console.log('generated', path.relative(repoRoot, eventConstantsOut));
  } catch (err) {
    console.error('event-constants codegen failed:', err.message);
    process.exitCode = 1;
    return;
  }
