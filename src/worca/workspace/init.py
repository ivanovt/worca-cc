...
Runs ``worca init --upgrade`` on each workspace repo in parallel.
...
def init_workspace_targets(
    *,
    ...
    """Run ``worca init --upgrade`` on each repo with per-target timeout.

    Args:
        repos: List of dicts with at least ``name`` and ``path`` keys.
    ...
    Returns:
        ``{repo_name: {"status": str, "reason": str | None}}``
...
def _init_single_target(
    *,
    ...
    cmd = [sys.executable, "-m", "worca", "init", repo_path, "--upgrade"]
